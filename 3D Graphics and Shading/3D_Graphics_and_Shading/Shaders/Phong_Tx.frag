...
uniform bool floor_effect = false; 
uniform float floor_frequency = 6.0f;
uniform float floor_width = 0.2f;
			...
			if (u_light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
				...
				if (tmp_float >= cos(radians(spot_cutoff_angle))) {
					if (!u_blind_effect) {
						...
						tmp_float = sin(70.0f * asin(tmp_float));
						if (tmp_float < zero_f) 
							tmp_float = zero_f;
					}
				}
...
//      fog_factor = (FOG_FAR_DISTANCE + v_position_EC.z)/(FOG_FAR_DISTANCE - FOG_NEAR_DISTANCE);
	...
	if (floor_effect) {
		float x_mod, y_mod;
		x_mod = mod(v_tex_coord.x*floor_frequency, 1.0f);
		y_mod = mod(v_tex_coord.y*floor_frequency, 1.0f);

		if ( (x_mod > floor_width) && (x_mod < 1.0f - floor_width) && 
		     (y_mod > floor_width) && (y_mod < 1.0f - floor_width) ) 
			discard;
	}
